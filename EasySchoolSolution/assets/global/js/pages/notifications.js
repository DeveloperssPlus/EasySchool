...
function generate(position, container, content, confirm ,x) {
 

    $('.preview .alert').removeClass('alert-success').removeClass('alert-info').removeClass('alert-danger').removeClass('alert-warning').addClass('alert-' + x);

    console.log('position: ' + position + ',container: ' + container + ', style: ' + 'topbar');
    //if(positipositionon == 'bottom') {
    ...
    if(container == '') {
        ...
        var n = noty({
            ...
            timeout: method,
            buttons     : confirm ? [
                //{addClass: 'btn btn-primary', text: 'd', onClick: function ($noty) {
                //    $noty.close();
                ...
                //}
                //},
                {addClass: 'btn btn-danger', text: '<i class="fa fa-close"></i> أغلق', onClick: function ($noty) {
                    $noty.close();
        ...
        var n = $(container).noty({
            ...
            maxVisible  : 10,
            buttons     : confirm ? [
                //{addClass: 'btn btn-primary', text: 'äÚã', onClick: function ($noty) {
                //    $noty.close();
                //    noty({dismissQueue: true, layout: 'topRight', theme : 'defaultTheme', text: 'You clicked "Ok" button', animation   : {
                //    open  : 'animated bounceIn', close : 'animated bounceOut'},type: 'success', timeout:3000});
                //}
                //},
                {addClass: 'btn btn-danger', text: 'ÅáÛÇÁ', onClick: function ($noty) {
                    $noty.close();
                //    noty({dismissQueue: true, layout: 'topRight', theme : 'defaultTheme',text: 'You clicked "Cancel" button', animation   : {
                //    open  : 'animated bounceIn', close : 'animated bounceOut'}, type: 'error', timeout:3000});
                }
